...
def _format_summary(agent_move: int, human_move: int, agent_reward: float, info: Dict, done: bool) -> str:
    ...
    lines = [
        f"PPO played **{_format_move(agent_move)}**.",
        f"You played **{_format_move(human_move)}**.",
        f"Rewards → PPO: {agent_reward:+}, You: {-agent_reward:+}.",
    ]
    winner = info.get("winner")
    if winner:
        winner_label = "PPO agent" if winner == "player1" else "You"
...
def play_turn(choice: str, session: ModelSession) -> Tuple[str, str, ModelSession]:
    """Handle a human move triggered from the UI."""
    if session is None:
