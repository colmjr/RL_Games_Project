...
from Tictactoe_env import CustomEnvironment, GRID_HEIGHT
...
class RandomOpponent:
    ...
    def __call__(self, obs=None, env: CustomEnvironment = None):
        try:
            ...
            x,y = int(np.random.randint(0, GRID_HEIGHT))
            return x, y
...
class SingleAgentEnv(gym.Env):
    ...
    def __init__(self, maxsteps: int, opponent_policy):
        ...
        self.observation_space = gym.spaces.Box(low=0.0, high=1.0, shape=(GRID_HEIGHT * GRID_HEIGHT,), dtype=np.float32)
        self.action_space = gym.spaces.Discrete(GRID_HEIGHT * GRID_HEIGHT)
        self._last_obs = None
    ...
    def step(self, action: int):
        ...
        x = int(action) // GRID_HEIGHT
        y = int(action) % GRID_HEIGHT
        agent_move = (x, y)
